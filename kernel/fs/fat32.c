...
static void build_demo_image(void) {
    ...
    const char* text =
        ...
        "Use: ls, cat README.TXT, pwd, mounts\n";
...
int fat32_delete_file(fat32_fs_t* fs, uint32_t dir_cluster, const char* name) {
    ...
    if (find_entry_in_dir(fs, dir_cluster, name, entry, &entry_sector, &entry_off) != 0) {
        return -2;
    }
    if (entry[11] & FAT32_ATTR_DIR) {
        return -3;
    ...
    first_cluster = ((uint32_t)rd16(&entry[20]) << 16) | (uint32_t)rd16(&entry[26]);
    if (first_cluster >= 2) {
        free_cluster_chain(fs, first_cluster);
    }

    sec = sector_ptr_mut(fs, entry_sector);
    if (!sec) {
        return -4;
    }

    mem_zero(sec + entry_off, 32);
    sec[entry_off] = 0xE5;
    return 0;
}
