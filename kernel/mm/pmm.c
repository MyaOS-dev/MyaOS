...
#define PMM_MAX_PAGE_FRAMES (PMM_MAX_PHYS_ADDR / PAGE_SIZE)
...
static int32_t g_free_page_pos[PMM_MAX_PAGE_FRAMES];
...
static uint64_t page_frame_index(uint64_t page_addr) {
    return align_down(page_addr, PAGE_SIZE) / PAGE_SIZE;
}

static void remove_free_page_at(uint64_t index) {
    uint64_t removed_page;
    uint64_t removed_pfn;
    uint64_t last_index;
    uint64_t moved_page;
    uint64_t moved_pfn;
    ...
    removed_page = g_free_pages[index];
    removed_pfn = page_frame_index(removed_page);
    last_index = g_free_top - 1u;
    moved_page = g_free_pages[last_index];
    moved_pfn = page_frame_index(moved_page);

    g_free_top--;
    if (index != last_index) {
        g_free_pages[index] = moved_page;
        if (moved_pfn < PMM_MAX_PAGE_FRAMES) {
            g_free_page_pos[moved_pfn] = (int32_t)index;
        }
    }
    if (removed_pfn < PMM_MAX_PAGE_FRAMES) {
        g_free_page_pos[removed_pfn] = -1;
    }
}
...
void pmm_init(const boot_info_t* boot, uint64_t kernel_start, uint64_t kernel_end) {
    ...
    g_allocated_pages = 0;
    g_managed_pages = 0;
    for (uint64_t i = 0; i < PMM_MAX_PAGE_FRAMES; i++) {
        g_free_page_pos[i] = -1;
    }
    ...
    while (offset + boot->desc_size <= boot->mmap_size) {
        ...
        if (desc->type == EFI_CONVENTIONAL_MEMORY) {
            ...
            for (uint64_t page = 0; page < desc->number_of_pages; page++) {
                ...
                {
                    uint64_t pfn = page_frame_index(page_addr);
                    if (pfn < PMM_MAX_PAGE_FRAMES) {
                        g_free_page_pos[pfn] = (int32_t)(g_free_top - 1u);
                    }
                }
                g_managed_pages++;
            }
...
uint64_t pmm_alloc_page(void) {
    ...
    page_addr = g_free_pages[g_free_top - 1u];
    remove_free_page_at(g_free_top - 1u);
    g_allocated_pages++;
    return page_addr;
}
...
uint64_t pmm_alloc_pages(uint64_t count) {
    ...
    for (uint64_t pfn = start_pfn; pfn + count <= PMM_MAX_PAGE_FRAMES; pfn++) {
        uint8_t found = 1u;

        for (uint64_t page = 0; page < count; page++) {
            if (g_free_page_pos[pfn + page] < 0) {
                found = 0;
                break;
            }
        }
        ...
        for (uint64_t page = 0; page < count; page++) {
            uint64_t run_pfn = pfn + page;
            int32_t index = g_free_page_pos[run_pfn];
            if (index >= 0) {
                remove_free_page_at((uint64_t)index);
            }
        }
...
void pmm_free_page(uint64_t page_addr) {
    uint64_t aligned = align_down(page_addr, PAGE_SIZE);
    uint64_t pfn;

    if (aligned == 0 || g_free_top >= PMM_MAX_TRACKED_PAGES) {
        return;
    }
    pfn = page_frame_index(aligned);
    if (pfn >= PMM_MAX_PAGE_FRAMES) {
        return;
    }
    if (g_free_page_pos[pfn] >= 0) {
        return;
    }

    g_free_pages[g_free_top] = aligned;
    g_free_page_pos[pfn] = (int32_t)g_free_top;
    g_free_top++;
...
void pmm_free_pages(uint64_t page_addr, uint64_t count) {
    if (page_addr == 0 || count == 0) {
        return;
    }

    for (uint64_t page = 0; page < count; page++) {
        pmm_free_page(page_addr + page * PAGE_SIZE);
    }
}
